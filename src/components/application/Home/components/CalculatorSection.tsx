...
const CalculatorSection = () => {
  ...
  const [constructionTypes, setConstructionTypes] = useState<
    ConstructionType[]
  >(constructionTypesData);
  const [buildPackages, setBuildPackages] =
    useState<BuildPackage[]>(buildPackagesData);
  const [investmentLevels, setInvestmentLevels] =
    useState<InvestmentLevel[]>(investmentLevelsData);
  const [coefficients, setCoefficients] =
    useState<Coefficient[]>(coefficientsData);
  ...
  useEffect(() => {
    if (constructionTypes.length > 0)
      setConstructionType(constructionTypes[0]._id);
    if (buildPackages.length > 0) setBuildPackage(buildPackages[0]._id);
    if (investmentLevels.length > 0)
      setInvestmentLevel(investmentLevels[0]._id);
  }, []);
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        const frontageCoefficient =
          additionalFactors.frontage[
            frontage as keyof typeof additionalFactors.frontage
          ] || 1.0;
        const alleyCoefficient =
          additionalFactors.alley[
            alley as keyof typeof additionalFactors.alley
          ] || 1.0;

        // Tính đơn giá sau khi áp dụng hệ số bổ sung
        const adjustedUnitPrice =
          baseUnitPrice * frontageCoefficient * alleyCoefficient;
        ...
        if (basement !== "Không hầm") {
          const basementDepthCoefficient =
            additionalFactors.basement_depth[
              basement as keyof typeof additionalFactors.basement_depth
            ] || 1.5;
                    ...
                    <div className="mt-4 p-6 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-200">
                          ...
                          <p className="text-2xl font-bold">
                            {result.unit_price.toLocaleString()} VNĐ/m²
                          </p>
                          <div className="mt-1 text-xs text-gray-500">
                            {frontage === "2" && (
                              <span className="inline-block mr-2 bg-blue-100 text-blue-800 px-1 rounded">
                                Mặt tiền: +5%
                              </span>
                            )}
                            {alley === "2" && (
                        ...
                        <div className="p-4 bg-white rounded-lg border border-primary text-primary">
                          ...
                          <p className="text-2xl font-bold">
                            {result.total_cost.toLocaleString()} VNĐ
                          </p>
                          <p className="text-xs mt-1">
                            {
                              constructionTypes.find(
                                (t) => t._id === constructionType
                              )?.name
                            }{" "}
                            -{" "}
                            {
                              buildPackages.find((p) => p._id === buildPackage)
                                ?.name
                            }{" "}
                            -{" "}
                            {
                              investmentLevels.find(
                                (l) => l._id === investmentLevel
                              )?.name
                            }
                          </p>
                        </div>
                      ...
                      <div className="mt-4 text-sm text-gray-600">
                        ...
                        <ul className="list-disc pl-5 space-y-1">
                          <li>
                            Báo giá trên là ước tính sơ bộ dựa trên thông tin
                            bạn cung cấp.
                          </li>
                          <li>
                            Chi phí thực tế có thể thay đổi tùy theo điều kiện
                            công trình và yêu cầu cụ thể.
                          </li>
                          <li>
                            Để nhận báo giá chính xác, vui lòng liên hệ với đội
                            ngũ tư vấn của chúng tôi.
                          </li>
                        </ul>
                      </div>
                    </div>
